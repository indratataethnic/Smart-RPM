...
export default function App() {
  ...
  // Verification Portal State (from QR code scan)
  const [verificationCode, setVerificationCode] = useState<string | null>(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      return params.get('v') || params.get('verify') || params.get('id') || params.get('doc');
    }
    return null;
  });
  ...
  useEffect(() => {
    const handlePopState = () => {
      const params = new URLSearchParams(window.location.search);
      setVerificationCode(params.get('v') || params.get('verify') || params.get('id') || params.get('doc'));
    };
  ...
  // If user opened a verification QR link, render VerificationPage
  if (verificationCode) {
    return (
      <VerificationPage
        ...
        {generatedPlan ? (
          ...
          <form onSubmit={handleGeneratePlan} noValidate className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-slate-900 via-teal-950 to-slate-900 text-white p-6 sm:p-8 rounded-3xl shadow-xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto shrink-0 flex-wrap">
                <button
                  type="button"
                  onClick={() => handleLoadDemoPreset(2)}
                  className="w-full sm:w-auto px-3.5 py-2 rounded-xl text-xs font-bold bg-emerald-800 hover:bg-emerald-700 text-emerald-100 border border-emerald-500/40 transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-sm"
                  title="Isi contoh otomatis untuk Koding dan Kecerdasan Artifisial (KKA) SD Fase C"
                >
                  <Cpu className="w-3.5 h-3.5 text-emerald-300" />
                  <span>Isi Contoh KKA SD (Fase C)</span>
                </button>
                <button
                  type="button"
